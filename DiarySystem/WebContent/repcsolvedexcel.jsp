...
try
{
	...
	<%
		sql="select * from t_callocation1 where  VehAvailDate >='"+fromdate+"' and VehAvailDate <='"+todate+"' and Status='Solved' and Tno in(select Tno from t_complaints1 where TdyDate between '"+fromdate+"' and '"+todate+"' and  RegBy='"+caller+"' and inspection='No') ";
		ResultSet rst=st.executeQuery(sql);
		System.out.println("sql--csolved-->" +  sql);
		int i=1;
		while(rst.next())
		{
		%>
	...
	%>
		...
		else
		{
			...
			<%
				sql="select distinct(VehRegNo) as VehRegNo,Status,VehAvailDate,ActualProb,Tno,DofAllocation from t_callocation1 where VehAvailDate between '"+fromdate+"' and '"+todate+"' and Status='Solved'  and Tno in(select Tno from t_complaints1 where TdyDate between '"+fromdate+"' and '"+todate+"' and  RegBy ='"+caller+"' and inspection='No') group by VehRegNo";
				ResultSet rst=st.executeQuery(sql);
				System.out.println("sql--csolved distinct-->" +  sql);
				int i=1;
				while(rst.next())
				{
				%>
