...
try
{
	...
	try{
	String sql1="update db_gps.t_ComplaintDetail set  Status='Solved'  where ComplaintID='"+RId+"'	";
	...
	String issue="",date="",report="",Website="",mailto="",EntBy="",Category="",ContactNumber="",ClosingComment,ClosingTime="",priority="-",ResponseTime="-";
	...
	try{
		...
		if(resoldate!=null)	
		{
		String sql2="insert db_gps.t_ComplaintDetailHistory(Description,PageName,ReportName,Website,ClosingComment,ClosedBy,Transporter,FileName,ComplaintDateTime,Category,Priority,Email1,EmailID,EntBy,ComplaintID,Name,updateComment,Status,ComplaintResolutionTime)values('"+issue+"','"+pagenm+"','"+report+"', '"+Website+"',  '"+clsdcmnt+"', '"+clsdby+"','"+transporter+"','"+flnm+"','"+date+"','"+Category+"','"+priority+"','"+mailto+"','"+email11+"','"+enby+"','"+RId+"','"+userName+"','"+Comments+"','Solved' ,'"+resoldate+"')	";
		//System.out.println("@@@@@@@@@@@@>>>>>>>>>>>>>:"+sql2);
		int j=st2.executeUpdate(sql2);
		
		}else{
			
			String sql2="insert db_gps.t_ComplaintDetailHistory(Description,PageName,ReportName,Website,ClosingComment,ClosedBy,Transporter,FileName,ComplaintDateTime,Category,Priority,Email1,EmailID,EntBy,ComplaintID,Name,updateComment,Status)values('"+issue+"','"+pagenm+"','"+report+"', '"+Website+"',  '"+clsdcmnt+"', '"+clsdby+"','"+transporter+"','"+flnm+"','"+date+"','"+Category+"','"+priority+"','"+mailto+"','"+email11+"','"+enby+"','"+RId+"','"+userName+"','"+Comments+"','Solved')	";
			//System.out.println("@@@@@@@@@@@@>>>>>>ELSE>>>>>>>:"+sql2);
			int j=st2.executeUpdate(sql2);
	...
	try{
		//email id who escalated the complaint
	...
	String body="<html><head><title>Mail Body</title></head><body><table border=^18^ bordercolor=^#E6E6FA^ align=^center^ width=^30%^><tr align=^center^><td align=^left^ style=^border: 0px;^ width=^100%^><table  style=^margin-left: 1em;margin-right: 1em;width: 90%;height: 100%;^><tr><td width=^100%^><a href=^http://www.mobile-eye.in^ target=^_blank^><img src=^http://192.168.2.55:8080/DiarySystem/images/mobile.jpg^ style=^height: 80px; width: 100px;^ title=^Mobile-eye^ id=^logo^ alt=^Logo^ /></a><br/><font style=^color: #D8D8D8^>___________________________________________________________________________</font><br/><br/><div>"
		 ...
		 +"<br>Your Complaint regarding complaint ID "+RId+" has been solved.Details of ticket are given below:"
      ...
      +ResponseTime+" </font></td></tr><tr><td width=^40%^ valign=^top^><font size=^2^ face=^Arial^>Solving comments: </font></td><td width=^60%^><font size=^2^ face=^Arial^>"+""
      +Comments+" </font></td></tr>";
	...
	if(i>0){
      ...
      try{
    	  ...
    	  String q = "insert into db_gps.t_allpendingmailtable(MailName,Description,MailBody,subjectline,Toid,Tocc,EntryDateTime,MailStatus,SenderName)values('Transworld','New  Complaint solved confirmation Email','"+body+"','Complaint solved - Transworld Compressor Technologies Ltd,Pune','"+escPersonEmail+"','"+entby+"','"+DateTime+"','Pending','Transworld')";
