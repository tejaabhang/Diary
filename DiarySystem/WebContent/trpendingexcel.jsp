...
try
{
	...
	<tr><td align="center"><font color="brown" size="2">Complaints pending for  All Transporter from <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate))%> to <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(todate))%> </font></td></tr>	
	<tr><td>
...
<td><b>Status</b></td>
	...
	<%
		sql="select * from t_callocation1 where Customer in (select distinct(OwnerName)as OwnerName from db_gps.t_vehicledetails where OwnerName not like '% del' order by OwnerName Asc)  and VehAvailDate >='"+fromdate+"' and VehAvailDate <='"+todate+"' and Status='Unsolved' and Tno in(select Tno from t_complaints1 where TdyDate between '"+fromdate+"' and '"+todate+"' and inspection='No' ) ";
	...
	%>
		...
		else
		{
			%>
			<table border="0" width="750px" align="center">
			<tr><td align="center"><font color="brown" size="2">Complaints pending by All Transporter  from <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate))%> to <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(todate))%> </font></td></tr>	
			<tr><td>
			<table border="1" width="100%" align="center" class="sortable">
			<tr>
			<td><b>Sr.</b></td>
	...
	<td><b>Vehicle Available Date </b></td>
			...
			<%
				sql="select distinct(VehRegNo) as VehRegNo,Status,VehAvailDate,ActualProb,Tno,DofAllocation from t_callocation1 where Customer in (select distinct(OwnerName)as OwnerName from db_gps.t_vehicledetails where OwnerName not like '% del' order by OwnerName Asc) and   VehAvailDate >='"+fromdate+"' and VehAvailDate <='"+todate+"' and Status='Unsolved' and  Tno in (select Tno from t_complaints1 where TdyDate between '"+fromdate+"' and '"+todate+"' and inspection='No')  group by VehRegNo";
				ResultSet rst=st.executeQuery(sql);
				//System.out.println("sql---->" +  sql);
				int i=1;
				while(rst.next())
