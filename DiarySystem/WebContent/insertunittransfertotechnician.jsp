...
try
{
	...
	//out.print(cntrint);
		...
		String[] tech = new String[cntrint];
		...
		for(int i=0; i<cntrint; i++) 
     		{
				...
				tech[i]=request.getParameter("tech"+i);
				...
				String sqlorder="insert into t_unitorder (Reqno, Reqto, ReqDate, TypofUnit, NoofUnits, RemUnits, Peripherals, InsType, DispatchAdd, Cust, Tech, DispDate, Status, ModeofDispatch, DisName, Reqby ) values ('"+orno+"','"+tech[i]+"', '"+tdydte+"', '"+typeunit+"', '-', '-', '"+periphs+"','"+neworrep+"','-','"+newcust+"','"+holdbyinsrt+"','"+tdydte+"','Transfer','Transfer','"+holdbyinsrt+"','"+user+"') ";
				//Query for t_sqlquery
		...
		response.sendRedirect("unitTransferToTechnician.jsp?updated=yes&holdername="+holdbyinsrt);
		return;
