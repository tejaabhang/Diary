...
String filename="PendingUnitsReport_For_Technician.xls";
    response.addHeader("Content-Disposition", "attachment;filename="+filename);
%>
<%@ include file="header.jsp" %>
<%! 
Connection con1, con2;
String username,technicians;
...
Statement sttech=con1.createStatement();
...
String sqltech="select distinct(TechName)as TechName,MobNo from t_techlist where Available='Yes'";
rstech = sttech.executeQuery(sqltech);
while(rstech.next())
{

	technicians=technicians+"','"+rstech.getString("TechName");
	...
	technicians=technicians+"')";
		...
		sql1="select * from t_unitreplacement where InstType In ('Rem','Rep','Del') and ( OldUnitWith = 'tech' or OldUnitWith in "+technicians+") and InstBy In "+technicians+" order by EntDate Desc";
...
sql66="select * from t_unitorder where Tech in "+technicians+" and Status <> 'Open' order by ReqDate desc ";
...
//sql77="select * from t_unitmaster where HoldBy in "+technicians+"";
//out.print(sql77);
//rs77=stmt7.executeQuery(sql77);
while(rs77.next())
{
	...
	sql88="select * from t_unitreplacement where NewUnitID='"+rs77.getString("UnitId")+"' and (InstDate > '"+rs77.getString("DispDate")+"' or EntDate > '"+rs77.getString("DispDate")+"') and InstBy In "+technicians+" order by EntDate Desc";
	rs88=stmt88.executeQuery(sql88);
	if(rs88.next())
	{}
	else
	{
		sql88="select * from t_unitreplacement where NewUnitID='"+rs77.getString("UnitId")+"' and (InstDate = '"+rs77.getString("DispDate")+"' or EntDate = '"+rs77.getString("DispDate")+"') and InstTime > '"+tme+"' and InstBy In "+technicians+" order by EntDate Desc";
