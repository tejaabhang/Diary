...
try{
	...
	String sql1="";
	int chkdcntr=0;
	String flag="true", mobsend="", idsend="", unitidsend="";
	...
	if(flag.equals("true"))
	{
   		response.sendRedirect("reprovisionrep.jsp?nocbselected=true");
	        return;
	}
	else
	{
		...
		for(int k=0;k<cntr;k++)
		{
			...
			if(x[k]==null)
				...
				mobno[k]=request.getParameter("mob"+k );
				unitid[k]=request.getParameter("unitid"+k ); 

				mobsend=mobsend+","+mobno[k];		
				idsend=idsend+","+id[k];
				unitidsend=unitidsend+","+unitid[k];
	
				sql1="update t_reprovisionnos set SentForReprov='Yes', ReprovisionDateTime='"+s+"' where SrNo='"+id[k]+"'";
		...
		//response.sendRedirect("reprovisionsentrep.jsp?mobsend="+mobsend+"&id="+idsend+"&units="+unitidsend);
		//return;

		response.sendRedirect("smails?mobsend="+mobsend+"&id="+idsend+"&units="+unitidsend);
		return;
