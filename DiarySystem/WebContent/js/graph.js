...
st += "<IMG SRC=graphimg/"+i+".gif BORDER=1 HSPACE=3>"+this.legends[i]+"<BR>\n";
}
st += "</FONT></TD></TR></TABLE>";
return st;
}
...
str += "<IMG BORDER=0 SRC=\"graphimg/"+i+".gif\" ";
str += "ALT=\"";
...
str += "<IMG SRC=\"graphimg/clear.gif\" WIDTH="+parseInt(g.hscale)+" HEIGHT=5>";
str += "</TD>\n<TD VALIGN=BOTTOM";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
str += ">";
}
str += "<IMG SRC=\"graphimg/clear.gif\" WIDTH=1 HEIGHT=5>";
}
str += "</TD>\n";
}
...
str += "<IMG SRC=\"graphimg/black.gif\" HEIGHT=1 WIDTH=";
...
for(var i = 0; i < g.rows.length; i++) {
if(parseInt(g.vscale*g.rows[i][j]) < 0) {
str += "<IMG VSPACE=0 HSPACE=0 BORDER=0 ALIGN=TOP SRC=graphimg/"+i+".gif WIDTH="+
...
str += "<IMG SRC=\"graphimg/clear.gif\" ALIGN=TOP BORDER=0 WIDTH="+parseInt(g.hscale)+
" HEIGHT=5>";
str += "</TD>\n<TD VALIGN=TOP";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
str += ">";
}
str += "<IMG SRC=\"graphimg/clear.gif\" ALIGN=TOP WIDTH=1 BORDER=0 HEIGHT=5>";
}
str += "</TD>\n";
if(g.legends && g.posMax == 0) {
...
st += "<IMG SRC=graphimg/black.gif HEIGHT=10 WIDTH=1>";
st += "<IMG SRC=graphimg/clear.gif HEIGHT=1 WIDTH=1>";
var n = g.rows[0].length;
var mult = g.rows.length;
if(g.stacked || g.relative) mult = 1;
for(var i = 0; i < n; i++) {
st += "<IMG SRC=graphimg/clear.gif HEIGHT=1 WIDTH="+(g.hscale*mult)+">";
if((i+1) % g.skip)
st += "<IMG SRC=graphimg/clear.gif HEIGHT=10 WIDTH=1>";
else
st += "<IMG SRC=graphimg/black.gif HEIGHT=10 WIDTH=1>";
}
st += "</TD></TR><TR>\n";
...
st += "<IMG SRC=graphimg/clear.gif WIDTH=1 HEIGHT="+H+"><BR>\n";
}
st += "</TD>\n";
st += "<TD VALIGN=TOP>";
for(var i = 0; i < n; i++) {
st += "<IMG SRC=graphimg/clear.gif WIDTH=1 HEIGHT="+(h-1)+"><BR>\n";
st += "<IMG SRC=graphimg/black.gif WIDTH=6 HEIGHT=1><BR>\n";
}
st += "</TD>\n";
st += "<TD VALIGN=TOP>";
st += "<IMG SRC=graphimg/black.gif WIDTH=1 HEIGHT="+(g.height*p)+">";
st += "<IMG SRC=graphimg/clear.gif WIDTH=1 HEIGHT="+(g.height*p)+">";
st += "</TD>\n"
return st;
}
...
str += "<IMG SRC=graphimg/0.gif WIDTH="+parseInt(g.hscale)+" HEIGHT="+(g.height-k)+" ";
str += "ALT=\"";
...
if(parseInt(g.vscale*g.rows[i][j]) > 0) {
str += "<IMG SRC=graphimg/"+i+".gif WIDTH="+parseInt(g.hscale)+" HEIGHT="+
...
if(!drawn) str += "<IMG SRC=graphimg/clear.gif WIDTH="+parseInt(g.hscale)+" HEIGHT=1>";
str += "</TD>\n";
str += "<TD";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
str += "><IMG SRC=graphimg/clear.gif WIDTH=1 HEIGHT=5></TD>\n";
}
}
if(g.legends && g.posMax != 0) {
str += "<TD WIDTH=5 NOWRAP ROWSPAN=3></TD>\n<TD ROWSPAN=3>";
