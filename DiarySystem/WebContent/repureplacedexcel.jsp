...
try
{
	...
	<%
		sql="select *  from db_gps.t_unitreplacement where EntBy='"+caller+"' and InstDate >='"+fromdate+"' and InstDate <='"+todate+"' and InstType='Rep'";
		ResultSet rst=st1.executeQuery(sql);
		System.out.println("sql-utreplaced all--->" +  sql);
		int i=1;
		while(rst.next())
		{
		%>
		...
		<td align ="right"><%=new SimpleDateFormat("dd-MMM-yyyy").format(rst.getDate("InstDate"))%></td>
	...
	%>
	   ...
	   <td align ="left"><%=rst.getString("EntBy") %></td>
		...
		else
		{
			...
			<%
				sql="select distinct(VehRegNo) as VehRegNo,EntBy,InstPlace,OwnerName,InstDate,OldUnitId,NewUnitId,InstBy from db_gps.t_unitreplacement where EntBy='"+caller+"' and InstDate >='"+fromdate+"' and InstDate <='"+todate+"' and InstType='Rep' group by VehRegNo";
				ResultSet rst=st2.executeQuery(sql);
				System.out.println("sql--utreplaced distinct--->" +  sql);
				int i=1;
				while(rst.next())
				{
				%>
