...
Statement st,stmt3, stmtsp1,stmtsp11,stmtsp111,stmt1,stmt2,stmtsp, st1,st23, st2, st3,st4,st5,st6, st9, st11, st01, st12,stamc,stamc1 ,stwar1,stwar2,stwar3,stwar4,stbtr;
String sql,sql1,sql2,sql3,sql4,sql5,sql6,sql7,sql8 , sqlbt;
...
%>
	<table border="0" width="750px" bgcolor="#E6E6E6" align ="center">
		<tr bgcolor="#BDBDBD" align ="center">
			<td align ="center" colspan="10">
				<font color="#2A0A12" size="3"><b>Battery Backup details</b></font>
	...
	<table border="1" align="center" width="750px" class="sortable">
		...
		try
		{
			...
			String us=username.toUpperCase();
			 String unitid="",group1="", vehregno="",loc="" ,transp="";
			//System.out.println("transporter--->"+transporter);
			
			sqlbt="select  UnitID, bcharging, bdatetime , VehicleRegNo  ,Location ,Transporter ,LatitudePosition ,LongitudePosition ,TheDate ,TheTime from t_onlinedata where  bcharging ='OFF' ";
		 	
		 	ResultSet rsbt=stbtr.executeQuery(sqlbt);
		 	System.out.println("sqlbt---->"+sqlbt);
		 	while(rsbt.next())
		 	{
		 		unitid=rsbt.getString("UnitID");
		 		battrystatus =rsbt.getString("bcharging");
				battrystatustime =rsbt.getString("bdatetime");
		 		
				vehregno=rsbt.getString("VehicleRegNo");
		 		
				 loc=rsbt.getString("Location");
				 transp=rsbt.getString("Transporter");
	...
	<td align="left"><div align="left"><%=Instruction%></div></td>
	<td  align="left"> <div align="left"><font color="maroon"> <%=battrystatus%>	</font> </div> </td>
		
		<td align="right"> <div align="left"><font color="maroon"><%if(battrystatustime.equalsIgnoreCase("NA")) { %> <%=battrystatustime %><%}else{ %> <%=new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(battrystatustime)) %><%} %>	</font> </div> </td>
	<td  align="right"><div align="left"><%=vehregno%>
	</div>
...
<%
	...
	<td align="left"><div align="left"><%=transp %></div></td>
		...
		if(rst1.next())
		{
			...
			upddate=rsbt.getString("TheDate");
...
%>
	<td ><div align="right"><%= new SimpleDateFormat("dd-MMM-yyyy").format(rsbt.getDate("TheDate"))%><br> <%=rsbt.getString("TheTime")%></div></td>

	<td><a href="# " onclick="window.open('shownewmap.jsp?lat=<%=rsbt.getDouble("LatitudePosition") %>&long=<%=rsbt.getDouble("LongitudePosition")%>' ,'m','width=500,height=500'); popWin.focus(); return false"><b> <font color ="" ><%=loc%> </font> </b> </a> <font color="red"> <%=msg %> </font></td>  
<% 
		}else{
			%>
			<td ><div align="right"><% try{
	...
	%></td>
	<td align="left"><div align="left"><%=rsJRMVehicles.getString("InstalledPlace")%> <font color="red"> In Active
			<%
			msg="In Active";
...
<%	}
	...
	//--------------------------------------------------------	
			...
			if(availdate.equals(""))
			{
				...
				<td align="right"><div align="left"><%=availdate %></div> </td>
				<% 
			}
	...
	<td align="left"><div align="left"><%=group1%></div></td>
	<%
	 }
	%>
	<td ><div align="right"><%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(instdte))%></div></td>
	<td align="left"><div align="left"><%=InstalledPlace%></div></td>
...
//	}
//	 %>
	<td align="left"><div align="left"><%=InstPlace%></div></td>
<%
...
%>
	...
	<td ><div align="left"> <a href="#" onClick="window.open ('spcmntabtveh.jsp?vehno=<%=vehregno%>&unitid=<%=unitid%>','win1','width=780,height=300,location=0,menubar=0,scrollbars=1,status=0,toolbar=0,resizable=0')"><font color="Red" size="2"> Click (<%=spcnt%>)</font></a>  </div></td>
	<td align="left"><div align="left"><%=spcmmnt %></td>
		<td align="left"><div align="left"><%=extracmnt %></div></td>
		<%
		...
		%>
		<td ><div align="right"> <%=new SimpleDateFormat("dd-MMM-yyyy hh:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse(Entrydt))%> </div></td>
		<%}else{
			System.out.println("3333333333!!>>>");
		%>
		<td align="left"><div align="right"><%=Entrydt %></div></td>
		<%} %>
		<td align="left"><div align="left"><%=Enteredby %></div></td>
	...
	<td align="left"><div align="left"><%=CodeVersion%></div></td>
		<td align="left"><div align="right"><%=WMSN%></div></td>
	...
	%>
	<td align="right"><div align="right"><%=psno %></div></td>
	<td align="right"><div align="right"><%=pcbno %></div></td>
	<td align="right"><div align="right"><%=boxno %></div></td>
	<td align="right"><div align="right"><%=batteryno %></div></td>
