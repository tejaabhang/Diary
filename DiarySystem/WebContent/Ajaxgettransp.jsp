...
Statement stmt1=con1.createStatement(), stmt11=con1.createStatement(), stmt2=con2.createStatement(), stmt3=con2.createStatement();
...
java.util.Date d = new java.util.Date();
			...
			String dte=formatter1.format(d);



if(firstfunc==null)
{
	String dispdate1="", recentinstdate1="", orderno1="", dispcust1="", loc1="", faulty1="", ownname1="", rdate1="",status1="";
	int days1=0;
	...
	sql4="select * from t_unitmasterhistory where UnitId='"+unitid1+"' and Status in ('Dispatched','Sent') order by DispDate desc limit 1";
	...
	String sqlVehicleAvailable="select * from t_vehicledetails where UnitID='"+unitid1+"'";
	...
	String sqlstatus="select * from t_unitreplacement where NewUnitId='"+unitid1.trim()+"' and EntDate between '"+dispdate1+"' and '"+dte+"' order by EntDate desc limit 1 ";
	ResultSet rsstatus=stmt1.executeQuery(sqlstatus);
	if(rsstatus.next())
	{
	}
		else
		//String transname1=rsstatus.getString("OwnerName");
		//status=rsstatus.getString("103.241.181.36");
		//if(status.equals("Rep") || status.equals("New"))
		{
			%>
				<font size="3" color="red"><b> Unconfigured unit </b></font> 
			<%
		}
...
String replace="select * from db_gps.t_unitreplacement where OldUnitID='"+unitid1+"' order by InstDate DESC limit 1";
ResultSet rsreplace=stmt3.executeQuery(replace);
//System.out.println(replace);
if(rsreplace.next()){
	unitlife=rsreplace.getString("UnitLife");
	iswarranty=rsreplace.getString("WasWarranty");
}else{
	unitlife="-";
	iswarranty="-";
}
...
%>
	...
	<td><b>Unit Life :</b><input type="text" name="unitlife" value="<%=unitlife%>" class="formElement" /></td>
	<td><b>Warranty :</b><input type="text" name="warranty" value="<%=iswarranty%>" class="formElement" /></td>	
<%	
}

else
{    
	String dispdate="", recentinstdate="", orderno="", dispcust="", loc="", faulty="", ownname="", rdate="", status="";
	int days=0;
	...
	sql4="select * from t_unitmasterhistory where UnitId='"+unitid+"' and Status in ('Dispatched','Sent') order by DispDate desc limit 1";
	...
	String sqlstatus="select * from t_unitreplacement where NewUnitId='"+unitid.trim()+"' and EntDate between '"+dispdate+"' and '"+dte+"' order by EntDate desc limit 1 ";
	ResultSet rsstatus=stmt1.executeQuery(sqlstatus);
	if(rsstatus.next())
	{
...
<%	}
String unitlife="",iswarranty="";
String replace="select * from db_gps.t_unitreplacement where OldUnitID='"+unitid+"' and InstDate >='"+dispdate+"' order by InstDate DESC limit 1";
ResultSet rsreplace=stmt3.executeQuery(replace);
//System.out.println(replace);
if(rsreplace.next()){
	unitlife=rsreplace.getString("UnitLife");
