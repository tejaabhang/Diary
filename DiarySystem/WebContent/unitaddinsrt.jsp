...
if(rs16.next())
{
   ...
   response.sendRedirect("addunit.jsp?dupveh="+vehreg+"&unidinveh="+unitid+"&ownernam="+ownnam);
   return;
}
...
/* ******** If all the conditions meet, it gets inserted in DB on both the servers************ */
	...
	rid=rid+1;
	sql5="insert into t_unitreplacement (VehCode, VehRegNo, OwnerName, NewUnitID, InstDate, InstTime, InstBy, InstPlace, InstType, EntBy, SimNo, MobNo, DriverName, DriverCode, DriverMobNo, TypeUnit, VoiceCallNo1, VoiceCallNo2, NewUnitRunOn, EntDate,adminoption) values ('"+nextvehcode+"', '"+vehreg+"', '"+ownname+"', '"+unid+"', '"+instdate+"', '"+instime+"', '"+instby+"', '"+locinsrt+"', 'New', '"+entby+"', '"+simno+"', '"+mobno+"', '"+drivname+"', '"+drivcode+"', '"+drivmob+"', '"+typeunit+"', '"+voice1+"', '"+voice2+"', '"+newunitrunon+"', '"+nwfrmtdte+"','"+whyoption+"')" ;
...
") ENGINE=MyISAM DEFAULT CHARSET=latin1";
	...
	*/
	...
	System.out.println("sqlfornd:-=============================================="+sqlfornd);
...
response.sendRedirect("addunit.jsp?send="+send+"&unitidnew="+unid);
return; 
