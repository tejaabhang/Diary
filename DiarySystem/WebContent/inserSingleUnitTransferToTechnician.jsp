...
try
{
	...
	//out.print(cntrint);
				 String unid=request.getParameter("unitid");
				...
				sql1="update t_unitmaster set TransDate='"+tdydte+"',OrderNo='"+orno+"', HoldBy='"+holdbyinsrt+"', CommentForTransf='"+comment+"', FaultyUnit='"+unittyp+"' where UnitID='"+unid+"' ";
				...
				String rno="0", simno="", mobno="", wmsn="", module="", gps="", neworrep="", typeunit="", swver="", voice="", jrmbuz="", buzzer="", temp1="", temp2="", door1="", door2="", horn="", scard="", flowsen1="", flowsen2="", plain="", periphs="", periphcode="0";  
				...
				String sqlorder="insert into t_unitorder (Reqno, Reqto, ReqDate, TypofUnit, NoofUnits, RemUnits, Peripherals, InsType, DispatchAdd, Cust, Tech, DispDate, Status, ModeofDispatch, DisName, Reqby ) values ('"+orno+"','"+tech+"', '"+tdydte+"', '"+typeunit+"', '-', '-', '"+periphs+"','"+neworrep+"','-','"+newcust+"','"+holdbyinsrt+"','"+tdydte+"','Transfer','Transfer','"+holdbyinsrt+"','"+user+"') ";
				//Query for t_sqlquery
				...
				sql3="insert into t_unitmasterhistory (Rno, EntDate, EntTime, UnitId, SimNo, MobNo, InstType, WMSN, Module, GPS, typeunit, SwVer, PVoice, PJrmBuzz, PBuzzer, PTemp1, PTemp2, PDoor1, PDoor2, PHorn, PScard, PFlowSen1, PFlowSen2, Plain, Peripherals, PeriphCode,Status,DisAdd, DispDate, DispTime, OrderNo, ModeofDispatch, DisName, CourierDate, HoldBy, FaultyUnit, CommentForTransf, EntBy, TransDate) values ('"+rno+"', '"+tdydte+"', '"+tme+"', '"+unid+"', '"+simno+"', '"+mobno+"', '"+neworrep+"', '"+wmsn+"', '"+module+"', '"+gps+"', '"+typeunit+"', '"+swver+"', '"+voice+"', '"+jrmbuz+"', '"+buzzer+"', '"+temp1+"', '"+temp2+"', '"+door1+"', '"+door2+"', '"+horn+"', '"+scard+"', '"+flowsen1+"', '"+flowsen2+"', '"+plain+"', '"+periphs+"', '"+periphcode+"','InterDisp','"+holdbyinsrt+"', '"+tdydte+"', '"+tme+"', '"+orno+"', 'Transfer','"+tech+"','"+tdydte+"', '"+holdbyinsrt+"', '"+unittyp+"', '"+comment+"','"+user+"', '"+tdydte+"') ";
				//Query for t_sqlquery
				...
				sql4="select * from t_unitmasterhistory where UnitId = '"+unid+"' and Status in ('Dispatched','InterDisp') order by DispDate Desc limit 1";
				System.out.print(sql3);
	   			String abcd4=sql4.replace("'","#");	
	   			abcd4=abcd4.replace(",","$");								
	   			stmtinsert.executeUpdate("Insert into t_sqlquery (dbname,query) values ('db_gps','"+abcd4+"')");
				//out.print(sql3);
				stmt3.executeQuery(sql4);
				
				String sql5="update t_unitmaster set RecConfirmByRep='"+entby+"',RecByTech='Yes',RecDateTime='"+tdydte+" "+tme+"' where UnitID='"+unid+"'";
				stmt3.executeUpdate(sql5);
				
				
				String sql6="insert into t_unitmasterhistory ("+
						...
						"Box_type,Tested_by,RecConfirmByRep) Select Rno,EntDate,EntTime,UnitID,SimNo,"+
						"SimCompany,MobNo,InstType,InstTypeNew,CommentOfEdit,"+
						"EditedBy,WMSN,Module,GPS,typeunit,"+
						"SwVer,PVoice,PJrmBuzz,PBuzzer,PTemp1,"+
						"PTemp2,PDoor1,PDoor2,PHorn,PScard,"+
						...
						"GPSMouseChanged,GSMAntennaChanged,SoftwareSimCorrections,OtherCorrections,TrackFileCount,"+
						"ManufactureCount,EntBy,TransDate,testedunit,Mobile_verified,"+
						"Box_type,Tested_by,RecConfirmByRep FROM t_unitmaster WHERE unitid = '"+unid+"'";
				
				stmt3.executeUpdate(sql6);
		con1.close();
		response.sendRedirect("singleUnitTransferToTechnician.jsp?updated=yes");
	return;
